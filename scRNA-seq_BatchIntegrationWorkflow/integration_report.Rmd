...
wrap_plots(rows, ncol = 1) + plot_annotation(title = "Figure 2. Integration visual comparison (UMAP)")
```
...
((p1 | p2) / (p3 | p4)) + plot_annotation(title = "Figure 3. Benchmarking metrics (subsampled)")
```
...
```{r save_outputs, eval=params$save_rds}
dir.create(dirname(params$out_prefix), showWarnings = FALSE, recursive = TRUE)

if (params$save_rds) {
  saveRDS(merged, paste0(params$out_prefix, "_merged.rds"))
  if (exists("seurat_int")) saveRDS(seurat_int, paste0(params$out_prefix, "_seurat_integrated.rds"))
  if (exists("harmony_int")) saveRDS(harmony_int, paste0(params$out_prefix, "_harmony.rds"))
  if (exists("fastmnn_int")) saveRDS(fastmnn_int, paste0(params$out_prefix, "_fastmnn.rds"))
}

write.csv(metrics$summary, paste0(params$out_prefix, "_metrics.csv"), row.names = FALSE)
write.csv(decision, paste0(params$out_prefix, "_decision.csv"), row.names = FALSE)
